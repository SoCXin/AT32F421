...
#define EXAMPLE_DATE "2021年4月15日"
...
int main(void)
{
    ...
    while (1)
    {
        if (ucSys10msFlag)
        {
            ucSys10msFlag = 0;
            /* 按键驱动 */
            ucKeyData = bsp_KEY_Driver();
            if (ucKeyData == S_KEY) /* 单键 */
            {
                printf("->按键单击\r\n");
            }
            else if (ucKeyData == L_KEY) /* 长键 */
            {
                printf("->按键长按\r\n");
            }
            /* 读取时钟 */
            ...
            if (ucSecondsOld != ERTC_TimeStructure.ERTC_Seconds)
            {
                ...
                printf("->芯片温度: %.3f℃\r\n", (ADC_TEMP_BASE - (double)usTemperatureAdc * ADC_VREF / 4096) / ADC_TEMP_SLOPE + 25);
